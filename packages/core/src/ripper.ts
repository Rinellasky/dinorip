...
export interface PolygonRipper {
  ...
  // points[i] to points[(i+1)%4]; `[C1, C2]` are the two control points, or
...
// The two cubic controls for edge `edgeIndex` (from points[i] to points[(i+1)%4]).
...
export function edgeControls(ripper: PolygonRipper, edgeIndex: number): [Vec2, Vec2] {
  const p0 = ripper.points[edgeIndex]!;
  const p3 = ripper.points[(edgeIndex + 1) % 4]!;
  const stored = ripper.edgeCurves?.[edgeIndex];
...
// A point on the ripper's warped surface (Coons patch) at output coords
...
// Whether this ripper should be extracted as a shape-preserving cutout. Curved
// rippers conserve by default; the flag (toggled per-texture in the atlas) can
// turn it off to fall back to rectification.
export function shouldConserve(ripper: PolygonRipper): boolean {
  return isRipperCurved(ripper) && (ripper.conserveShape ?? true);
}

// Flatten the ripper boundary (its four cubic edges) into a polygon. Shared by
// the CPU and GPU conserve paths and by hit-testing so they all agree.
export function ripperOutlinePoints(ripper: PolygonRipper, segmentsPerEdge = 8): Vec2[] {
  const outline: Vec2[] = [];
  for (let edge = 0; edge < 4; edge += 1) {
    const p0 = ripper.points[edge]!;
    const p3 = ripper.points[(edge + 1) % 4]!;
    const [c1, c2] = edgeControls(ripper, edge);
    for (let step = 0; step < segmentsPerEdge; step += 1) {
      outline.push(cubicBezier(p0, c1, c2, p3, step / segmentsPerEdge));
    }
  }
  return outline;
}
